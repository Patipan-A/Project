...
# อ่านภาพและแปลงเป็นขาวเทา
watermark = Image.open('w.bmp').convert("L")
# แปลงภาพขาวเทาเป็นภาพไบนารี (ค่ามากกว่า 0.8 จะเป็น 1)
threshold = 0.8 * 255  # คูณด้วย 255 เพราะค่าที่ใช้ใน PIL คือ 0-255
binary_image = watermark.point(lambda p: 1.0 if p > threshold else 0.0)

# แปลงภาพไบนารีเป็น array ของ numpy
binary_array = np.array(binary_image)

# แปลงเป็นชนิดข้อมูล double
binary_double = binary_array.astype(np.float64)  # เปลี่ยนเป็นชนิดข้อมูล float64

# Flatten array ให้กลายเป็นเวกเตอร์
watermark_flat = binary_double.flatten()
matrix = watermark_flat
...
for i in range(len(matrix)):
    if i < half_length:
        first_half.append(matrix[i])
    else:
        second_half.append(matrix[i])
...
Id = np.argmax(TpTnpsnr[:, 2])  # ดัชนีของค่าสูงสุด

# เรียกใช้ฟังก์ชัน embeded_modification2
...
# เรียกใช้ฟังก์ชัน embeded_modification2 สำหรับ dot_data
...
# คูณด้วย 255 เพื่อแปลงกลับเป็นขอบเขต 0-255
embeded = (embeded * 255).astype(np.uint8)

# สร้างภาพจากอาเรย์ RGB
